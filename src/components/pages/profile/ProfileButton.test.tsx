...
mockUsePrimary.mockImplementation((address, skip) => {
  ...
  return {
    ...
    name: skip ? undefined : isNoPrimary ? null : 'primary.mxc',
    beautifiedName: isNoPrimary ? null : 'primary.mxc',
    loading: false,
  }
})
...
describe('<OwnerProfileButton/>', () => {
  ...
  describe('parent', () => {
    ...
    it('should display link to /tld/eth if value is tld', () => {
      ...
      expect(element).toHaveAttribute('href', '/tld/mxc')
    })
    ...
    it('should display link to /name.eth if value is not tld', () => {
      ...
      const element = screen.getByTestId('owner-profile-button-name.parent')
      expect(element).toBeInTheDocument()
      expect(element).toHaveTextContent(`name.parentname.mxc`)
      expect(element).toHaveAttribute('href', '/name.mxc')
    })
  })
})
