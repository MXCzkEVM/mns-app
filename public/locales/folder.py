...
openai.api_key = "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"
...
def translate_nested_dict(data, target_language):
    for key, value in data.items():
        if isinstance(value, dict):
            translate_nested_dict(value, target_language)
        else:
            data[key] = translate(value, target_language)
    ...
    for lang_code, lang_name in language_dict.items():
        ...
        for filename in os.listdir('en'):
            if filename.endswith(".json"):
                ...
                translate_nested_dict(data, lang_name)
